...
@Component
public class RemovedStatus implements UpdateClaimStatus {
    ...
    public ResponseEntity<?> updateTransferStatus(String claimUuid, String comment) {
        ...
        if (claim.getStatus() != ClaimStatus.REJECTED) {
            throw new BadRequestException("Only rejected claims can be removed");
        }

        claim.setStatus(ClaimStatus.REMOVED);
        claim.addLog(new ClaimLogs(claim, ClaimStatus.REMOVED, comment));
        claimRepository.save(claim);

        updateBatchStatus(claim.getBatchRecord());

        return ResponseEntity.ok(new MessageResponse("Claim removed successfully"));
    }
